...
var waterShader = new THREE.ShaderMaterial({
  uniforms: {
    uGlobalTime: {
      type: 'f',
      value: 0.1
  ...
  fragmentShader: [
    ...
    'vec3 getSkyColor(vec3 e) {',
      'e.g = max(e.g, 0.0);',
      'vec3 skyCol;',
      'skyCol.r = pow(1.0 - e.g, 2.0);',
      'skyCol.g = 1.0 - e.g;',
      'skyCol.b = 0.6+(1.0 - e.g) * 0.4;',
      'return skyCol;',
    '}',
    ...
    'float map(vec3 p) {',
      ...
      'for(int i = 0; i < ITER_GEOMETRY; i++) {',
        'd = sea_octave((uv + uGlobalTime) * freq, choppy);',
        'd += sea_octave((uv - uGlobalTime) * freq, choppy);',
        'h += d * amp;',
        'uv *= octave_m;',
        'freq *= 1.9;',
